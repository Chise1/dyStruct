...
`)
	...
	err = writer.LinkSet([]string{"SubStruct", "1"}, subStruct2{Index: "2"})
	if err != nil {
		t.Fatal(err)
	}
	assert.Equal(t, "&{123 example 123.45 true [1 2 3]  {0 } [{1} {2}]}", fmt.Sprintf("%v", instance))
	err = writer.LinkSet([]string{"SubStruct", "0"}, nil)
...
`)
	...
	err = writer.LinkSet([]string{"Struct", "Integer"}, 100)
	assert.Equal(t, nil, err)
	...
	err = writer.LinkSet([]string{"Struct"}, struct {
	...
	st, found := writer.LinkGet([]string{"Struct"})
	if !found {
...
`)
	...
	err = writer.LinkSet([]string{"SubStruct"}, subStruct{
		Boolean: false,
	})
	assert.Equal(t, nil, err)

	err = writer.LinkSet([]string{"SubStruct"}, subStruct2{
		Index: "10",
	})
	assert.NotEqual(t, nil, err)

	dd, _ := writer.LinkGet([]string{"SubStruct"})
	//marshal, _ := json.Marshal(instance)
	if fmt.Sprintf("%v", dd) != "{false}" {
		t.Fatal("not equal")
	}
	err = writer.LinkSet(
		[]string{"SubStruct"}, subStruct{
			Boolean: true,
		})
	if err != nil {
		t.Fatal(err)
...
`)
	...
	err = writer.LinkSet([]string{"Slice"}, []int{4, 5, 6})
	if err != nil {
		t.Fatal(err)
	}
	...
	err = writer.LinkSet([]string{"Slice", "*"}, 7)
	err = writer.LinkSet([]string{"Slice", "*"}, 8)
	err = writer.LinkSet([]string{"Slice", "*"}, 9)
	...
	err = writer.LinkSet([]string{"Slice", "0"}, nil)
	assert.Equal(t, nil, err)
	err = writer.LinkSet([]string{"Slice", "0"}, nil)
	assert.Equal(t, nil, err)
	err = writer.LinkSet([]string{"Slice", "0"}, nil)
...
func TestSubSliceFinal(t *testing.T) {
	...
	assert.Equal(t, nil, err)
	err = wt.LinkSet([]string{"Index"}, 11)
	assert.Equal(t, nil, err)
	...
	err = writer.LinkSet([]string{"SubStruct", "*"}, reflect.Indirect(reflect.ValueOf(sub1)).Interface())
	assert.Equal(t, nil, err)

	err = writer.LinkSet([]string{"Index"}, "111")
	assert.Equal(t, nil, err)
	...
	writer, err = NewWriter(instance)
	assert.Equal(t, nil, err)
	sl, found := writer.LinkGet([]string{"SubStruct2", "SubStruct", "0", "Index"})
	assert.Equal(t, true, found)
	assert.Equal(t, 2, sl)
	err = writer.LinkSet([]string{"SubStruct2", "SubStruct", "0", "Index"}, 3)
	assert.Equal(t, nil, err)
	sl, found = writer.LinkGet([]string{"SubStruct2", "SubStruct", "0", "Index"})
	assert.Equal(t, true, found)
	assert.Equal(t, 3, sl)
...
func TestMap(t *testing.T) {
	...
	writer, err := NewWriter(instance)
	assert.Equal(t, nil, err)
	err = writer.LinkSet([]string{"Map", "text1", "Integer"}, 2)
	...
	ret, found := writer.LinkGet([]string{"Map", "text1", "Integer"})
	assert.Equal(t, true, found)
	assert.Equal(t, 2, ret.(int))
	err = writer.LinkSet([]string{"Map", "text2"}, struct {
		Integer int    `json:"int"`
		Text    string `json:"someText"`
	}{22, "text2"})
	assert.Equal(t, nil, err)
	...
	err = writer.LinkSet([]string{"Map", "text1"}, nil)
	assert.Equal(t, nil, err)
	bytes, err = json.Marshal(instance)
...
func TestSliceMap(t *testing.T) {
	...
	assert.Equal(t, nil, err)
	err = writer.LinkSet([]string{"Sub", "10"}, struct {
		Index int
		Name  string
	}{10, "shi"})
	assert.Equal(t, nil, err)
	assert.Equal(t, "{0 [{10 shi}]}", fmt.Sprint(s1))
	err = writer.LinkSet([]string{"Sub", "20"}, struct {
		Index int
		Name  string
	}{20, "ershi"})
	assert.Equal(t, nil, err)
	assert.Equal(t, "{0 [{10 shi} {20 ershi}]}", fmt.Sprint(s1))
	assert.Equal(t, "{20 ershi} true", fmt.Sprint(writer.LinkGet([]string{"Sub", "20"})))
	assert.Equal(t, `not found field Integer2`, writer.LinkSet([]string{"Integer2"}, 2).Error())
	assert.Equal(t, "<nil> false", fmt.Sprint(writer.LinkGet([]string{"Integer2"})))
}
...
func TestSliceSub(t *testing.T) {
	...
	writer, err := NewWriter(&s1)
	assert.Equal(t, nil, err)
	err = writer.LinkSet([]string{"Sub", "1"}, struct {
		Index int
		Name  string
	}{10, "shi"})
	assert.Equal(t, nil, err)
	assert.Equal(t, "{0 [{0 } {10 shi}]}", fmt.Sprint(s1))
	err = writer.LinkSet([]string{"Sub", "3", "Name"}, "30")
	assert.Equal(t, nil, err)
	assert.Equal(t, "{0 [{0 } {10 shi} {0 } {0 30}]}", fmt.Sprint(s1))
	err = writer.LinkSet([]string{"Sub", "3", "Index"}, "30")
	assert.NotEqual(t, nil, err)
	assert.Equal(t, "{0 [{0 } {10 shi} {0 } {0 30}]}", fmt.Sprint(s1))
	err = writer.LinkSet([]string{"Sub", "3", "Index"}, 30)
	assert.Equal(t, nil, err)
	assert.Equal(t, "{0 [{0 } {10 shi} {0 } {30 30}]}", fmt.Sprint(s1))
	err = writer.LinkSet([]string{"Sub", "x", "Index"}, 30)
	assert.NotEqual(t, nil, err)
	index, found := writer.LinkGet([]string{"Sub", "4", "Index"})
	assert.Equal(t, false, found)
	assert.Equal(t, nil, index)
	d, f := writer.LinkGet([]string{"Sub", "3"})
	...
	val, found = writer.LinkGet([]string{"Sub"})
	assert.Equal(t, true, found)
	assert.Equal(t, "[{10 f10}]", fmt.Sprint(val))
	err = writer.LinkSet([]string{"Sub", "0"}, struct {
		Index int
		Name  string
	}{100, "x100"})
	assert.Equal(t, nil, err)
	val, found = writer.LinkGet([]string{"Sub", "x"})
	assert.Equal(t, nil, val)
	assert.Equal(t, false, found)
	val, found = writer.LinkGet([]string{"Sub"})
	assert.Equal(t, "[{100 x100}]", fmt.Sprint(val))
	err = writer.LinkSet([]string{"Sub", "1", "Index"}, 200)
	assert.Equal(t, nil, err)
...
func TestMapImpl(t *testing.T) {
	...
	assert.Equal(t, nil, err)
	err = writer.LinkSet([]string{"M3"}, map[string]string{
		"zhangsan": "ei",
	})
	assert.Equal(t, nil, err)
	assert.Equal(t, "{0 map[] map[zhangsan:ei]}", fmt.Sprint(s))
	err = writer.LinkSet([]string{"M2", "1"}, sub{Index: 1, Name: "lisi"})
	assert.Equal(t, nil, err)
	...
	val, found := writer.LinkGet([]string{"M2", "1", "Index"})
	assert.Equal(t, true, found)
	assert.Equal(t, 1, val)
	_, found = writer.LinkGet([]string{"M2", "3"})
	assert.Equal(t, false, found)
	val, found = writer.LinkGet([]string{"M2", "1"})
	assert.Equal(t, true, found)
	assert.Equal(t, "{1 lisi}", fmt.Sprint(val))
	val, found = writer.LinkGet([]string{"M2"})
	assert.Equal(t, true, found)
	assert.Equal(t, "map[1:{1 lisi}]", fmt.Sprint(val))
	data := `{"Index":33,"Name":"ff33"}`
	err = UpdateFromJson(writer, []string{"M2", "33"}, []byte(data), json.Unmarshal, nil)
	assert.Equal(t, nil, err)
...
func TestMapSlice(t *testing.T) {
	...
	writer, err := NewWriter(&s)
	assert.Equal(t, nil, err)
	err = writer.LinkSet([]string{"M"}, map[string]string{
		"zhangsan": "lisi",
	})
	assert.Equal(t, nil, err)
	assert.Equal(t, "{map[zhangsan:lisi]}", fmt.Sprint(s))
	err = writer.LinkSet([]string{"M", "wangmazi"}, "fff")
	assert.Equal(t, nil, err)
	assert.Equal(t, "{map[wangmazi:fff zhangsan:lisi]}", fmt.Sprint(s))
}
